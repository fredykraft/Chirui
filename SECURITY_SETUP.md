...
1. `ADMIN_PASSWORD.md` - Local-only admin notes (no plaintext credentials)
2. `ADMIN_ANALYTICS_GUIDE.md` - Local-only setup notes
...
- Replace `[YOUR_PASSWORD_HINT_HERE]` with a non-sensitive reminder only
...
❌ **NEVER commit:**
- `ADMIN_PASSWORD.md` (local notes only)
- `ADMIN_ANALYTICS_GUIDE.md` (local notes only)
...
4. Update local notes only (do not store plaintext passwords)
